...
// Auth disabled: no session available by default
...
export default function Dashboard() {
  ...
  useEffect(() => {
    // Fetch API keys immediately; Authorization header will be omitted when session is null.
    fetchApiKeysData();
  }, []);
  ...
  const handleCreateApiKey = async (name, limit) => {
    try {
      const headers = { 'Content-Type': 'application/json', ...(session?.accessToken ? { 'Authorization': `Bearer ${session.accessToken}` } : {}) };
